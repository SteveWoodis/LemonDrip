...
const CC_0 = 0x30, CC_9 = 0x39;
const CC_SPACE = 0x20, CC_TAB = 0x09, CC_LF = 0x0a, CC_CR = 0x0d;
const CC_MINUS = 0x2d, CC_PLUS = 0x2b, CC_DOT = 0x2e;
const CC_QUOTE = 0x22, CC_BACKSLASH = 0x5c;
const CC_COMMA = 0x2c, CC_COLON = 0x3a;
const CC_LBRACE = 0x7b, CC_RBRACE = 0x7d;
const CC_LBRACKET = 0x5b, CC_RBRACKET = 0x5d;
const CC_E_LOWER = 0x65, CC_E_UPPER = 0x45;
const CC_T = 0x74, CC_R = 0x72, CC_U = 0x75, CC_E = 0x65; // 'true'
const CC_F = 0x66, CC_A = 0x61, CC_L = 0x6c, CC_S = 0x73; // 'false'
const CC_N = 0x6e; // 'null'
const isWhitespace = (c) => c === CC_SPACE || c === CC_TAB || c === CC_LF || c === CC_CR;
const isDigit = (c) => c >= CC_0 && c <= CC_9;
 ...
 * @returns JSON string
 */
const toJson = (data, replacer, space) => {
    // Use native JSON.stringify with a custom replacer for BigInt
    const preliminaryJSON = JSON.stringify(data, (key, value) => {
...
function fromJson(json, reviver) {
    ...
    function skipWhitespace() {
        while (i < len && isWhitespace(json.charCodeAt(i)))
            i++;
    }
    function parseValue() {
        skipWhitespace();
        ...
        while (i < len) {
            ...
            if (c === CC_BACKSLASH) {
                ...
                while (i < len) {
                    ...
                    if (cc === CC_QUOTE || cc === CC_BACKSLASH)
                        break;
                    i++;
                }
    ...
    function parseNumber() {
        ...
        if (json.charCodeAt(i) === CC_0) {
            ...
            while (i < len && isDigit(json.charCodeAt(i)))
                i++;
        }
        const intEnd = i;
        // Fraction part
    ...
    if (!reviver)
        return result;
    function reviveRecursive(holder, key) {
        let value;
        ...
        return reviver === null || reviver === void 0 ? void 0 : reviver.call(holder, key, value);
    }
    return reviveRecursive({ "": result }, "");
}
