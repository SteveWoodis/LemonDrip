...
Object.entries(raw).forEach(([key, value]) => {
  if (CANONICAL_KEYS.has(key)) {
    // Direct match: raw key is already camelCase (e.g. "eventFee")
    canonical[key] = value;
  } else {
    // Label-derived key: "Event_Fee" → "Event Fee" → CANONICAL_LABEL_TO_DBKEY → "eventFee"
    const labelKey = key.replace(/_/g, " ");
    const dbKey = CANONICAL_LABEL_TO_DBKEY[labelKey];
    if (dbKey && CANONICAL_KEYS.has(dbKey)) {
